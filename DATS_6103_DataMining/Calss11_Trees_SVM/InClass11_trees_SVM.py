...
import numpy as np
import pandas as pd
os.chdir('../Class11_Trees_SVM')
filename = 'gradAdmit.csv'
# # 
# # filepath = os.path.join( os.getcwd() ,filename )
# # dfadmit = pd.read_csv(filepath) 
dfadmit = pd.read_csv(filename) 
dfChkBasics(dfadmit)
...
# but then, home brew no longer allows sudo
...
# Run the file 
# plot_iris_svc 
# (need make_meshgrid and plot_contours functions there)
# 

try:
    make_meshgrid
except NameError:
    raise Exception("\n\nSTOP! \nYou need to open the file \nplot_iris_svc.py \nand load the functions there.\n\n")
else:
    print("\nReady to continue.")
...
# Run the file 
# classifierPlots.py
# (need plot_classifier, plot_classifiers, and plot_4_classifiers functions there)
# 
try:
    plot_classifier
except NameError:
...
svc_cv_acc = cross_val_score(svc, X_train, y_train, cv= 10, scoring='accuracy', n_jobs = -1)
...
y_pred = dtree_admit1.predict(X_test)
# Evaluate test-set accuracy
print(accuracy_score(y_test, dtree_admit1.predict(X_test)))
print(confusion_matrix(y_test, dtree_admit1.predict(X_test)))
print(classification_report(y_test, dtree_admit1.predict(X_test)))
